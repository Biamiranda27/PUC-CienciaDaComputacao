...
void selecao_lista(Lista* lista, int* comparacoes, int* movimentacoes) {
    ...
    for (Celula* i = lista->primeiro->prox; i->prox != NULL; i = i->prox) {
        ...
        for (Celula* j = i->prox; j != NULL; j = j->prox) {
            (*comparacoes)++;
            if (strcmp(menor->restaurante->nome, j->restaurante->nome) > 0) {
                menor = j;
            }
        }
        if (menor != i) {
...
int main() {
    ...
    selecao_lista(&selecionados, &comparacoes, &movimentacoes);
    clock_t fim = clock();
    ...
    FILE* log = fopen("882406_selecao_flexivel.txt", "w");
    if (log != NULL) {
        fprintf(log, "882406\t%d\t%d\t%.2f\n", comparacoes, movimentacoes, tempo_execucao);
        fclose(log);
    }

    liberar_lista(&selecionados);
