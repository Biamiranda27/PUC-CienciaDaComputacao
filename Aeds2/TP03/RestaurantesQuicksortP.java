...
class Restaurante {
    ...
    public Restaurante(int id, String nome, String city, int capacidade, double avaliacao,
        ...
        this.id = id; this.nome = nome; this.city = city; this.capacidade = capacidade;
    ...
    public String getCidade() { return city; }
    ...
    public String formatar() {
        ...
        return String.format("[%d ## %s ## %s ## %d ## %.1f ## %s ## %s ## %s-%s ## %s ## %s]",
                id, nome, city, capacidade, avaliacao, tc, fp,
...
public class RestaurantesQuicksortP {
    static long comparacoes = 0;
    static long movimentacoes = 0;

    static int compararRestaurantes(Restaurante a, Restaurante b) {
        comparacoes++;
    ...
    static void quicksortParcialRec(Restaurante[] v, int esq, int dir, int k) {
        if (esq >= dir) return;

        int i = esq, j = dir;
        Restaurante pivo = v[(esq + dir) / 2];
        movimentacoes++;

        while (i <= j) {
            while (compararRestaurantes(v[i], pivo) < 0) {
                i++;
            }
            while (compararRestaurantes(v[j], pivo) > 0) {
                j--;
            }
            if (i <= j) {
                Restaurante tmp = v[i];
        ...
        if (esq < j) {
            quicksortParcialRec(v, esq, j, k);
        }
        if (i < dir && i < k) {
            quicksortParcialRec(v, i, dir, k);
        }
    }

    static void quicksortParcial(Restaurante[] v, int n, int k) {
        quicksortParcialRec(v, 0, n - 1, k);
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        quicksortParcial(selecionados, count, k);
        long tempo = System.nanoTime() - inicio;
        for (int i = 0; i < count; i++)
            System.out.println(selecionados[i].formatar());
        String matricula = "845860";
        try (FileWriter fw = new FileWriter(matricula + "_quicksort_parcial.txt")) {
