...
void ler_csv_colecao(Colecao_Restaurantes* colecao, const char* path) {
...
void insercao_parcial(Restaurante** array, int n, int k) {
    for (int i = 1; i < n; i++) {
        ...
        if (i >= k) {
            comparacoes++;
            if (strcmp(tmp->cidade, array[k - 1]->cidade) >= 0) {
                continue;
            }
        }
        ...
        while (j >= 0) {
            comparacoes++;
            if (strcmp(tmp->cidade, array[j]->cidade) < 0) {
                array[j + 1] = array[j];
                movimentacoes++;
                j--;
            } else {
...
int main() {
    ...
    insercao_parcial(selecionados, n_selecionados, k);
    ...
    FILE* log = fopen("845860_insercao_parcial.txt", "w");
    if (log != NULL) {
        fprintf(log, "845860\t%lld\t%lld\t%lld\n", comparacoes, movimentacoes, tempo);
        fclose(log);
    }
