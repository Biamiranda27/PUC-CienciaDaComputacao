    ...
    if (original && original->elements) {
        ...
        if (newArray.elements) {
            for (int i = 0; i < newArray.count; i++) {
                newArray.elements[i] = strdup(original->elements[i]);
            }
        }
    }
    return newArray;
...
StringArray parseStringArray(char* data, const char* delimiter) {
    ...
    if (working_data[0] == '[' && working_data[strlen(working_data)-1] == ']') {
        ...
        char *temp = working_data + 1;
        memmove(working_data, temp, strlen(temp) + 1);
    }
    ...
    char* saveptr;
    char* split_data = working_data;
    
    token_to_store = strtok_r(split_data, delimiter, &saveptr);
    while (token_to_store != NULL) {
        char* cleaned = strdup(token_to_store);
        int len = strlen(cleaned);
        int start = 0;
        int end = len - 1;
        while(start < len && isspace((unsigned char)cleaned[start])) start++;
        while(end >= 0 && isspace((unsigned char)cleaned[end])) end--;
        if (start <= end) {
            cleaned[end + 1] = '\0';
            arr.elements[i++] = strdup(cleaned + start);
        } else {
            arr.elements[i++] = strdup("NaN");
        }
        free(cleaned);
        token_to_store = strtok_r(NULL, delimiter, &saveptr);
    }
...
ReleaseDate parseDate(const char* dateStr) {
    ...
    if (dateStr == NULL || strcmp(dateStr, "NaN") == 0) {
        return date;
    }
...
void Games_lerCSV(Games* game, char* linha) {
    ...
    free(game->App_Id);
    game->App_Id = strdup(fields[0]);
    
    free(game->Name);
    game->Name = strdup(fields[1]);

    game->Release_Date = parseDate(fields[2]);

    free(game->Estimated_Owners);
    game->Estimated_Owners = strdup(fields[3]);
    ...
    free(game->Publishers);
    game->Publishers = strdup(fields[9]);

    free(game->Developers);
    game->Developers = strdup(fields[10]);

    freeStringArray(&game->Categories);
    game->Categories = parseStringArray(fields[11], ",");

    freeStringArray(&game->Genres);
    game->Genres = parseStringArray(fields[12], ",");
    
    freeStringArray(&game->Tags);
    game->Tags = parseStringArray(fields[13], ",");
...
char* find_line_by_id(const char* id) {
    for (int i = 0; i < csv_count; i++) {
        if (strcmp(ArquivoCSV[i].id, id) == 0) {
            return ArquivoCSV[i].line;
        }
    }
    return NULL;
}
...
int main() {
    ...
    if (file == NULL) {
        perror("Error opening /tmp/games.csv");
        return 1;
    }
    ...
    if (ArquivoCSV == NULL) {
        perror("Memory allocation failed for CSV records");
        fclose(file);
        return 1;
    }
    ...
    while (fgets(input, sizeof(input), stdin) != NULL) {
        ...
        if (csv_line != NULL) {
            Games_lerCSV(&game, csv_line);
            Games_imprimirCSV(&game);
        } else {
            printf("=> NaN ## NaN ## NaN ## NaN ## 0.0 ## [NaN] ## -1 ## -1.0 ## 0 ## [NaN] ## [NaN] ## [NaN] ## [NaN] ## [NaN] ##\n");
        }
    }

    Games_free(&game);
    for (int i = 0; i < csv_count; i++) {
        free(ArquivoCSV[i].id);
        free(ArquivoCSV[i].line);
    }
    free(ArquivoCSV);
