/*Usando vetores, implemente o algoritmo Quicksort considerando que a chave de ordenação é o atributo Release_date.
Em caso de empate, utilize como critério secundário o AppID do Game.

Entrada e saída padrão são iguais às da questão de Ordenação por Seleção.
Saída: os registros ordenados pelo atributo Release_date (desempatando por AppID).

 Além disso, crie um arquivo de log na pasta corrente com o nome suamatricula_quicksort.txt, contendo uma única linha com:
...
Tempo de execução do algoritmo de ordenação
 ...
 Todas as informações do arquivo de log devem ser separadas por uma tabulação (\t).*/#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdbool.h>
#include <ctype.h>
...
int compareGames(Games a, Games b) {
    comparacoes++;
    if (a.Release_Date.isValid && b.Release_Date.isValid) {
        if (a.Release_Date.year != b.Release_Date.year)
            return (a.Release_Date.year < b.Release_Date.year) ? -1 : 1;
        if (a.Release_Date.month != b.Release_Date.month)
            return (a.Release_Date.month < b.Release_Date.month) ? -1 : 1;
        if (a.Release_Date.day != b.Release_Date.day)
            return (a.Release_Date.day < b.Release_Date.day) ? -1 : 1;
    }
    return atoi(a.App_Id) - atoi(b.App_Id);
}

void swap(Games* a, Games* b) {
    Games temp = *a;
    *a = *b;
    *b = temp;
    movimentacoes += 3;
...
int partition(Games* array, int low, int high) {
    Games pivot = array[(low + high) / 2];
    int i = low - 1, j = high + 1;
    while (1) {
        do { i++; } while (compareGames(arr[i], pivot) < 0);
        do { j--; } while (compareGames(arr[j], pivot) > 0);
        if (i >= j) return j;
        swap(&array[i], &arr[j]);
    }
}

void quick(Games* arr, int low, int high) {
    if (low < high) {
