...
public class RestaurantesInsercao {
    public static void main(String[] args) {
        ...
        for (int i = 1; i < numSelecionados; i++) { //ordena por insercao, sendo que ve a cidade por ordem alfabetica e se for igual usa o id
            Restaurante tmp = selecionados[i];
            int j = i - 1;
            
            while (j >= 0) {
                comparacoes++;
                int cmp = selecionados[j].getCidade().compareTo(tmp.getCidade());
                ...
                if (cmp > 0) {
                    ...
                    comparacoes++;
                    if (selecionados[j].getId() > tmp.getId()) {
                        selecionados[j + 1] = selecionados[j];
                        movimentacoes++;
                        j--;
                    } else {
        ...
        try (PrintWriter out = new PrintWriter(new FileWriter("882406_insercao.txt"))) {
