...
public class RestaurantesPSequencial {
    public static void main(String[] args) {
        ...
        while (scanner.hasNextLine()) {
            ...
            // Busca sequencial no array de selecionados
            for (int i = 0; i < numSelecionados; i++) {
                comparacoes++;
                if (selecionados[i].getNome().equals(nomeBusca)) {
                    encontrado = true;
                    break;
                }
            }
        ...
        try (PrintWriter out = new PrintWriter(new FileWriter("882406_sequencial.txt"))) {
            out.printf("882406\t%d\t%d\n", (tempoFim - tempoInicio), comparacoes);
        } catch (IOException e) {
