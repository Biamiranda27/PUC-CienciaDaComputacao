...
void limpar_string(char* string) {
    ...
    while (string[inicio] == ' ' || string[inicio] == '\"' || string[inicio] == '[' || string[inicio] == '\'') inicio++;
    // CORREÇÃO: "str[fim]" alterado para "string[fim]"
...
Data parse_data(char* string) { 
    Data data = {0, 0, 0};
    if (string == NULL || strlen(string) == 0 || strcmp(string, "NaN") == 0) {
        return data;
    }
...
Hora parse_hora(char* string) {
    ...
    // CORREÇÃO: "&hhora.hora" alterado para "&hora.hora"
...
Restaurante* parse_restaurante(char* string) {
    ...
    if (strlen(campos[5]) > 0) {
        ...
        while (token != NULL) {
            limpar_string(token);
            // CORREÇÃO: "r->n_tipos_cozinha" alterado para "restaurante->n_tipos_cozinha"
